...
| `js/habitat.js` | 生態全景場景(地面、草叢、花、棲木、天空、霧)+ 昆蟲擺放與待機漫遊 + **散佈複本 `decor`**(每區 clone 該區真實物種 ~24 份、`clone(true)` 共用幾何/材質、依 `decorWeight` 小蟲多大蟲少;隨 simT 漫遊+拍翅、可點選→聚焦該種,讓畫面佈滿該區昆蟲) |
