...
- **兩層視角**:
  - **生態全景**:一片盛夏草地,八種台灣常見昆蟲在其中活動(空中飛的蝴蝶/蜻蜓、花上的蜜蜂/瓢蟲、葉間的螳螂/蝗蟲、地面的螞蟻、樹幹上的獨角仙)。可自由環顧、縮放,遠看整個生態。
  - **昆蟲特寫**:點任一昆蟲,相機飛近做特寫。可近看身體構造。
- **教學功能**:
  - 每種昆蟲有**資料卡**(真實數據、生命週期/變態、趣聞)。
  - **構造標註**:近看時開啟,在 3D 模型上標出頭/胸/腹/複眼/翅/足等部位並解說。
  - **昆蟲導覽**:自動巡覽八種昆蟲,依「分類故事」順序解說(鱗翅→鞘翅→膜翅→蜻蛉→…)。
  - **真實相對比例**:全景中八種昆蟲以真實體型比例並存(瓢蟲 7mm 遠小於蜻蜓 100mm),建立尺寸感。
- **八種昆蟲**:大紫蛺蝶、獨角仙、西方蜜蜂、無霸勾蜓、七星瓢蟲、寬腹螳螂、黑棘蟻、台灣大蝗。
...
| `js/insects.js` | 程序化昆蟲模型建構器(八種);每個回傳 `{group, anchors, animate, baseLength}` |
| `js/habitat.js` | 生態全景場景(地面、草叢、花、棲木、天空、霧)+ 八種昆蟲的擺放與待機漫遊 |
...
- 所以八種昆蟲在全景中是**真實體型比**(這是刻意的教學效果:瓢蟲就是比蜻蜓小一大截)。
