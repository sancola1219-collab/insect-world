...
- **2026-07-06 幼蟲/蛹階段模型重製(Fable 5)**:`lifecycle.js` 全面重寫,提升形似與質感。
  - 毛蟲:豐滿分節(radius 用 `plump(u)` 中段最粗)、頭殼+臉(複眼/大顎)、3 對真足+4 對腹足+尾足、背中線斑與側氣孔。
  - 雞母蟲:C 形肥胖、褐色硬頭殼+大顎、分節褶、半透尾端(內臟)、捲曲胸足。
  - 懸蛹:`LatheGeometry` 車床曲面(平滑優雅)、翅芽稜線、帝王蝶式金屬金點、懸絲+尾柄。
  - 裸蛹:看得出成形中的成蟲——頭胸膨大、背側翅芽、折疊觸角與三對足、向下微彎的分節腹。
  - 卵直立小叢+縱稜停於葉片;蛆逗點半透;鱷魚幼蟲側疣突+大顎+六足(瓢蟲橙疣/螢火蟲發光)。
  - 驗證:6 種 larva/pupa 組合 mesh 數 12–72、`lifecycle().sane` 全 true、adult 還原 true、console 無錯誤、可渲染(sampleRT 非零)。
